...
define('FORMAT_SINGLEACTIVITYPERPAGE_COLLAPSED', 1);
define('FORMAT_SINGLEACTIVITYPERPAGE_EXPANDED', 0);
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_singleactivityperpage extends format_base {
    /**
    ...
    public function section_format_options($foreditform = false) {
        return array(
            ...
            'collapsed' => array(
                'type' => PARAM_INT,
                'label' => get_string('displaycontent', 'format_singleactivityperpage'),
                'element_type' => 'select',
                'element_attributes' => array(
                    array(
                        FORMAT_FLEXSECTIONS_EXPANDED => new lang_string('showexpanded', 'format_singleactivityperpage'),
                        FORMAT_FLEXSECTIONS_COLLAPSED => new lang_string('showcollapsed', 'format_singleactivityperpage'),
                    )
                ),
     ...
     *
     * format_singleactivityperpage processes additional attributes in the view course URL
     ...
     *
     * @param int|section_info $section
     * @return array of format_singleactivityperpage_edit_control
     */
    public function get_section_edit_controls($section) {
        ...
        if ($sectionnum && has_capability('moodle/course:update', $context) && $sectionnum != $sr) {
            ...
            if ($section->collapsed == FORMAT_FLEXSECTIONS_EXPANDED) {
                $text = new lang_string('showcollapsed', 'format_singleactivityperpage');
                $class = 'expanded';
            } else {
                $text = new lang_string('showexpanded', 'format_singleactivityperpage');
                $class = 'collapsed';
            }
            $controls[] = new format_singleactivityperpage_edit_control($class, $switchcollapsedurl, $text);
        }
        ...
        if ($sectionnum && has_capability('moodle/course:setcurrentsection', $context)) {
            ...
            if ($course->marker == $section->section) {
                $marker = 0;
                $text = new lang_string('removemarker', 'format_singleactivityperpage');
                $class = 'marked';
            } else {
                $marker = $section->section;
                $text = new lang_string('setmarker', 'format_singleactivityperpage');
                $class = 'marker';
            }
            $setmarkerurl->params(array('marker' => $marker, 'sesskey' => sesskey()));
            $controls[] = new format_singleactivityperpage_edit_control($class, $setmarkerurl, $text);
        }
        ...
        if (has_capability('moodle/course:update', $context)) {
            ...
            $controls[] = new format_singleactivityperpage_edit_control('settings', $editurl, $text);
        }
        ...
        if ($sectionnum && has_capability('moodle/course:update', $context)) {
            ...
            $text = new lang_string('mergeup', 'format_singleactivityperpage');
            $controls[] = new format_singleactivityperpage_edit_control('mergeup', $mergeupurl, $text);
        }
        ...
        if ($sectionnum && has_capability('moodle/course:update', $context)) {
            ...
            $text = new lang_string('deletesection', 'format_singleactivityperpage');
            $controls[] = new format_singleactivityperpage_edit_control('delete', $deleteurl, $text);
        }
        ...
        if ($sectionnum && !$movingsection && has_capability('moodle/course:update', $context) && $sectionnum != $sr) {
            ...
            $controls[] = new format_singleactivityperpage_edit_control('move', $moveurl, $text);
        }
        ...
        if ($sectionnum && has_capability('moodle/course:sectionvisibility', $context)) {
            if ($section->visible) {
                ...
                $controls[] = new format_singleactivityperpage_edit_control('hide', $hideurl, $text);
            } else {
                if ($section->parent && !$this->get_section($section->parent)->visible) {
                    $controls[] = new format_singleactivityperpage_edit_control('show', null, '');
                } else {
                    $showurl = course_get_url($course, $sr);
                    $showurl->params(array('show' => $section->section, 'sesskey' => sesskey()));
                    $text = new lang_string('show');
                    $controls[] = new format_singleactivityperpage_edit_control('show', $showurl, $text);
                }
            }
        }
    ...
    public function get_edit_control_movehere($parent, $before, $sr = null) {
        ...
        return new format_singleactivityperpage_edit_control('movehere', $movelink, $str);
    }

    /**
     * Returns a control to exit the section moving mode
     *
     * @return null|format_singleactivityperpage_edit_control
     */
    public function get_edit_controls_cancelmoving() {
        ...
        if ($movingsection) {
            ...
            $str = strip_tags(get_string('cancelmoving', 'format_singleactivityperpage', $this->get_section_name($movingsection)));
            $controls[] = new format_singleactivityperpage_edit_control('cancelmovingsection', $cancelmovingurl, $str);
        }
        ...
        if (ismoving($this->courseid)) {
            ...
            $str = strip_tags(get_string('cancelmoving', 'format_singleactivityperpage', $USER->activitycopyname));
            $controls[] = new format_singleactivityperpage_edit_control('cancelmovingactivity', $cancelmovingurl, $str);
        }
        return $controls;
    }
    ...
    public function get_add_section_control($parentsection) {
        ...
        if ($parentsection) {
            $text = new lang_string('addsubsection', 'format_singleactivityperpage');
        } else {
            $text = new lang_string('addsection', 'format_singleactivityperpage');
        }
        return new format_singleactivityperpage_edit_control('addsection', $url, $text);
    }
    ...
    public function course_content_header() {
        ...
        if ($this->get_viewed_section()) {
            ...
            if ($section->parent) {
                ...
                $text = new lang_string('backtosection', 'format_singleactivityperpage', $this->get_section_name($section->parent));
            } else {
                $sr = 0;
                $text = new lang_string('backtocourse', 'format_singleactivityperpage', $this->get_course()->fullname);
            }
            $url = $this->get_view_url($section->section, array('sr' => $sr));
            return new format_singleactivityperpage_edit_control('backto', $url, strip_tags($text));
        }
        ...
        if ($PAGE->context && $PAGE->context->contextlevel == CONTEXT_MODULE && $PAGE->cm) {
            ...
            if ($sectionnum) {
                $text = new lang_string('backtosection', 'format_singleactivityperpage', $this->get_section_name($sectionnum));
            } else {
                $text = new lang_string('backtocourse', 'format_singleactivityperpage', $this->get_course()->fullname);
            }
            return new format_singleactivityperpage_edit_control('backto', $this->get_view_url($sectionnum), strip_tags($text));
        }
    ...
    public function inplace_editable_render_section_name($section, $linkifneeded = true,
        ...
        if (empty($edithint)) {
            $edithint = new lang_string('editsectionname', 'format_singleactivityperpage');
        }
        if (empty($editlabel)) {
            $title = get_section_name($section->course, $section);
            $editlabel = new lang_string('newsectionname', 'format_singleactivityperpage', $title);
        }
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_singleactivityperpage_edit_control implements renderable {
    public $url;
    public $text;
    public $class;
 ...
 * @return \core\output\inplace_editable
 */
function format_singleactivityperpage_inplace_editable($itemtype, $itemid, $newvalue) {
    global $DB, $CFG;
    require_once($CFG->dirroot . '/course/lib.php');
    if ($itemtype === 'sectionname' || $itemtype === 'sectionnamenl') {
        $section = $DB->get_record_sql(
            'SELECT s.* FROM {course_sections} s JOIN {course} c ON s.course = c.id WHERE s.id = ? AND c.format = ?',
            array($itemid, 'singleactivityperpage'), MUST_EXIST);
