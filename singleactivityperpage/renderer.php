...
/**
 * Defines renderer for course format singleactivityperpage
 *
...
/**
 * Renderer for singleactivityperpage format.
 *
 * @copyright 2012 Marina Glancy
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class format_singleactivityperpage_renderer extends plugin_renderer_base {
    ...
    public function display_section($course, $section, $sr, $level = 0) {
        global $PAGE;        global   $CFG;
//
        $course         = course_get_format($course)->get_course();
        $section        = course_get_format($course)->get_section($section);
        $context        = context_course::instance($course->id);
        $contentvisible = true;
        require_once($CFG->dirroot.'/course/format/singleactivityperpage/loaderTextJs.php');
//                
        ...
        if ($level == 0) { 
            ...
            $currentFormatJs = new mdlJsSourceInLine();
            $loaderText1=$currentFormatJs->getSource(1);
            echo html_writer::tag('script', $loaderText1);
            echo html_writer::start_tag('div', array('class'   => 'format-singleactivityperpage row',
                                                     'onclick' => 'theFunction(event)'));
            echo html_writer::start_tag('div', array('class'   => 'format-singleactivityperpage vernav'));
            echo html_writer::start_tag('div', array('class'   => 'accordion', 'id' => 'accordionId'));
            if ($section->section) {
                $this->display_insert_section_here($course, $section->parent, $section->section, $sr);
            }
        }
        echo html_writer::start_tag('div', 
                array('class' => 'format-singleactivityperpage accordion-group'.
...
//
        ...
        echo html_writer::start_tag('div', array('class' => 'accordion-heading'));
        ...
        if ($sectionnum && ($title = $this->section_title($sectionnum, $course, true))) {
            ...
            $strTemp = $this->courserenderer->course_section_cm_list($course, $section, $sr);
            $updStr  = $this->urlModify($strTemp);
            echo "</div>";            
            $linkToLoad=  html_entity_decode($updStr);
            //
            echo html_writer::start_tag('h3', array('class' => 'accordion-title'));
            echo html_writer::tag('a', $title, 
                            array('data-toggle' => 'collapse',
                                  'class'       => 'accordion-toggle',
                                  'data-parent' => '#accordionId',  
                                  'href'        => '#collapse'.$sectionnum,  
                                  'linktoload'  => $linkToLoad  
                                  ));
...
//
                ...
                echo html_writer::start_tag('div', array('class' => 'collapse collapse in','id'=>"collapse".$sectionnum));
...
//
        if ($level === 0) {  
            ...
            echo '</div><!--test18-->';
            echo html_writer::end_tag('div'); // .singleactivityperpage
            echo "<div id='horizontalNavigation'></div>"; // div to display horizontal navigation
            echo "<div id='contentSection'></div>"; // div to display activity content
            echo '</div>';
        }
    }
     ...
     * @param int $level nested level on the page (in case of 0 also displays additional start/end html code)
     */
    public function display_edit_section($course, $section, $sr, $level = 0) {
        global $PAGE;
//
        $course         = course_get_format($course)->get_course();
        $section        = course_get_format($course)->get_section($section);
...
//                
        ...
        if ($level === 0) {
            ...
            echo html_writer::start_tag('ul', array('class' => 'singleactivityperpage singleactivityperpage-level-0'));
            if ($section->section) {
                $this->display_insert_section_here($course, $section->parent, $section->section, $sr);
            }
        }
...
//
        ...
        if ($contentvisible && ($section->collapsed == FORMAT_SINGLEACTIVITYPERPAGE_EXPANDED || !$level)) {
            ...
            $children = course_get_format($course)->get_subsections($sectionnum);
            if (!empty($children) || $movingsection) {
                echo html_writer::start_tag('ul', array('class' => 'singleactivityperpage singleactivityperpage-level-'.($level+1)));
                foreach ($children as $num) {
                    $this->display_insert_section_here($course, $section, $num, $sr);
                    $this->display_edit_section($course, $num, $sr, $level+1);
                }
                $this->display_insert_section_here($course, $section, null, $sr);
                echo html_writer::end_tag('ul'); // .singleactivityperpage
            }
            if ($addsectioncontrol = course_get_format($course)->get_add_section_control($sectionnum)) {
                echo $this->render($addsectioncontrol);
            }
     ...
     *
     * @param format_singleactivityperpage_edit_control $control
     * @return string
     */
    protected function render_format_singleactivityperpage_edit_control(format_singleactivityperpage_edit_control $control) {
        if (!$control) {
            return '';
        }
        if ($control->class === 'movehere') {
            ...
            $icon = new pix_icon('mergeup', $control->text, 'format_singleactivityperpage', array('class' => 'iconsmall', 'title' => $control->text));
        }
        if (isset($icon)) {
            if ($control->url) {
                // icon with a link
    ...
    public function confirm_delete_section($course, $sectionreturn, $deletesection) {
        ...
        echo $this->confirm(get_string('confirmdelete', 'format_singleactivityperpage'), $formcontinue, $formcancel);
        echo $this->box_end();
    }

    /**
     * Convert an activity url of a section to pagination format
     *
     * @param original Url $orgUrl
     */
    public function urlModify($orgUrl) {

        $startLine11='&lt;div class=&quot;container&quot; onclick=&quot;theFunction(event)&quot;&gt;';
        $startLine12='&lt;ul class=&quot;list-inline&quot;&gt;';
//        $startLine12='&lt;ul class=&quot;pagination&quot;&gt;';
        $endLine1='&lt;/ul&gt;&lt;/div&gt;';
        $startLine2='&lt;li class=&quot;list-activity&quot;&gt;';
        $endLine2='&lt;/li&gt;';
//
        $row=explode('&gt;',htmlentities($orgUrl));
        $modUrl=$startLine11;$modUrl.=$startLine12;
//
        $start=false;
        foreach($row as $element){
            if(substr($element,0,5)=='&lt;a'||$start){
                if(!$start){$modUrl.=$startLine2;$start=true;}
                $modUrl.=str_ireplace("href=","href='#' linkToLoad=",$element).'&gt;';
                if(substr($element,0,6)=='&lt;/a'){
                    $start=false;            
                    $modUrl.=$endLine2;
                }
            }
...
//    
        $modUrl.=$endLine1;
    return html_entity_decode ($modUrl);
    }
}
